...
<div class="row" id="<?php echo 'RecordTable' . $MonthID; ?>">
	<div class="col-lg-12">
		<table width="100%" class="table table-striped table-bordered">
			<thead>
				<tr>
					<th colspan="7"><?php echo $FeeMonths[$MonthID]['MonthName'] . ' ( <small>' . $StudentDetailObject->GetFirstName() . ' ' . $StudentDetailObject->GetLastName() . '</small> ) '; ?>:</th>
				</tr>
				<tr>
					<th>Sr.No.</th>
					<th>Fee Head</th>
			...
			<tbody>
				...
				foreach ($StudentMonthlyFeeDetails as $StudentFeeStructureID => $MonthlyFeeDetail) {
					...
					$WaveOffAmount = $MonthlyFeeDetail['WaveOffAmount'];
					...
					$TotalWaveOffAmount += $WaveOffAmount;
					...
					echo '<td>' . (($WaveOffAmount) ? number_format($WaveOffAmount, 2).' <i class="fa fa-inr"></i>' : '') . '</td>';

					echo '<td><input type="text" class="form-control StudentFeeHeadAmount CheckedFeeHead' . $MonthID . '" id="HeadAmount' . $StudentFeeStructureID . '" name="txtStudentFeeHeadAmount[' . $StudentFeeStructureID . ']" value="' . ($FeeAmount - $DiscountAmount - $WaveOffAmount) . '"> </i>
					</td>';
					...
					echo '<td><label class="checkbox-inline"><input class="custom-radio WaveOff" type="checkbox" id="WaveOff' . $StudentFeeStructureID . '" name="chkWaveOff[' . $StudentFeeStructureID . ']" value="' . ($FeeAmount - $DiscountAmount - $WaveOffAmount) . '" /> OFF</td>';
					echo '</tr>';
				}

				if ($LastYearSubmitedFee > 0) {
					echo '<tr class="text-right">';
					echo '<td colspan="5"><strong>Last Year Submited Fee :</strong></td>';
				...
				echo '<td>' . number_format($TotalWaveOffAmount, 2) . '&nbsp;</td>';
