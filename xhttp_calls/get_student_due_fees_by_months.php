...
<?php 
		...
		foreach ($StudentMonthlyFeeDetails as $StudentFeeStructureID => $MonthlyFeeDetail) 
		{
			...
			echo '<tr class="text-right">';				
				...
				echo '<td>' . $MonthlyFeeDetail['ConcessionAmount'] . ' <i class="fa fa-inr"></i></td>';

				echo '<td>'. $MonthlyFeeDetail['AmountPaid'] .'</td>';
				echo '<td><input type="text" class="form-control StudentFeeHeadAmount CheckedFeeHead'. $MonthID .'" id="HeadAmount'. $StudentFeeStructureID .'" name="txtStudentDueFeeHeadAmount['. $StudentFeeStructureID .']" value="'. ($FeeAmount - $DiscountAmount - $MonthlyFeeDetail['AmountPaid'] - $MonthlyFeeDetail['ConcessionAmount']) .'"> </i>
					</td>';
