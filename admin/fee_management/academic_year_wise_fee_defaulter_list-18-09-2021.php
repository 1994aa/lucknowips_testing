...
switch ($Clean['Process']) {
	case 1:
		...
		//get records count
		FeeCollection::SearchFeeDefaulters1($TotalRecords, true, $Filters, $FeePriority);

		if ($TotalRecords > 0) {
			$DefaulterList = FeeCollection::SearchFeeDefaulters1($TotalRecords, false, $Filters, $FeePriority, 0, $TotalRecords);
	...
	case 7:
		...
		$Filters['DueFromDate'] = date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['DueFromDate']))));
		$Filters['DueToDate'] = date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['DueToDate']))));
		...
		if ($TotalRecords > 0) {
			...
			if (isset($_GET['report_submit']) && $_GET['report_submit'] == 2) {
				require_once('../excel/fee_defaulter_list_download_xls1.php');
			}
			if ($Clean['AllRecords'] == 'All') {
				$DefaulterList = FeeCollection::SearchFeeDefaulters1($TotalRecords, false, $Filters, $FeePriority, 0, $TotalRecords);
			} else {
				$DefaulterList = FeeCollection::SearchFeeDefaulters1($TotalRecords, false, $Filters, $FeePriority, $Start, $Limit);
			}
		}
		break;
}
...
<body>
	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			...
			<form class="form-horizontal" name="frmRoomReport" action="academic_year_wise_fee_defaulter_list.php" method="get">
					...
					<div id="collapseOne" class="panel-collapse collapse in">
						<div class="panel-body">
							...
							<div class="form-group">
								<label for="AcademicYear" class="col-lg-2 control-label">Previous Academic Year</label>
								<div class="col-lg-3">
			...
			if ($Clean['Process'] == 7 && $HasErrors == false) {
				...
				if ($Clean['DueFromDate'] != '') {
					$ReportHeaderText .= ' Due Between : ' . $Clean['DueFromDate'] . ' and ' . $Clean['DueToDate'] . ',';
				}
									...
									<div class="row" id="RecordTable">
										<div class="col-lg-12">
											<table width="100%" class="table table-striped table-bordered table-hover" id="DataTableRecords">
												<thead>
													<tr>
														...
														foreach ($FeeHeadList as $FeeHeadID => $FeeHeadDetails) {
															//$FeeHeadTotalDue is array for Grand Total of Fee Head Due
												...
												<tbody>
													<?php
													//$TotalDue is Grand Total Due Amount which consists of all types of dues
													//$TotalPreviousYearDue is Grand Total PreviousYearDue 
													$TotalDue = 0;
													...
													if (is_array($DefaulterList) && count($DefaulterList) > 0) {
														...
														foreach ($DefaulterList as $StudentID => $Details) {
															...
															//$StudentFeeHeadDueAmount is Student's Total Fee Head's Dues
															$StudentFeeHeadDueAmount = 0;
															$StudentFeeHeadDueAmountPreviousYear = 0;
															...
															//may be we can use this as a modal
												// 			echo '<pre>';
												// 			print_r($FeeDefaulterDues);exit;

															if ($Clean['AcademicYearID'] == 2) {
																$PreviousYearDueDetails = FeeCollection::GetPrevoiusYearDue($StudentID, 30, 1, '2019-04-01', '2020-03-31');
																
																// echo '';
																// print_r($PreviousYearDueDetails);exit;

																foreach ($FeeHeadList as $FeeHeadID => $FeeHeadDetails) {
																	$FeeHeadDueAmount = 0;
																	foreach ($PreviousYearDueDetails as $Month => $DefaulterDetails) {
																		if ($Clean['FeeHeadID'] > 0) {
																	...
																	if ($FeeHeadDueAmount > 0) {
																		$StudentFeeHeadDueAmountPreviousYear += $FeeHeadDueAmount;
																	}
																}

																# get previous year due amount which is paid in current year before filter last date

																$PreviousYearFeePaid = FeeCollection::GetPreviousYeatFeePaidByDate($StudentID, $DueToDate);
																//var_dump($PreviousYearFeePaid);
																$StudentFeeHeadDueAmountPreviousYear += $PreviousYearDue;
																$StudentFeeHeadDueAmountPreviousYear -= $PreviousYearFeePaid;
															} else {
																$StudentFeeHeadDueAmountPreviousYear = $PreviousYearDue + $PreviousDueAmount;
															}

															$HasAmount = false;

															if ($StudentFeeHeadDueAmountPreviousYear > 0) {
																$HasAmount = true;
															}
															...
															foreach ($FeeHeadList as $FeeHeadID => $FeeHeadDetails) {
																...
																foreach ($FeeDefaulterDues as $Month => $DefaulterDetails) {
																	if ($Clean['FeeHeadID'] > 0) {
																		if ($Clean['FeeHeadID'] == $FeeHeadID && array_key_exists($Clean['FeeHeadID'], $DefaulterDetails)) {
																			if ($DefaulterDetails[$Clean['FeeHeadID']]['FeeHeadAmount'] > 0) {
																				$HasAmount = true;
																			}
																		}
																	} else {
																		if (array_key_exists($FeeHeadID, $DefaulterDetails)) {
																			if ($DefaulterDetails[$FeeHeadID]['FeeHeadAmount']) {
																				$HasAmount = true;
																			}
																		}
																	}
													...
													?>
															<tr>
																...
																<td class="text-right"><?php echo number_format($StudentFeeHeadDueAmountPreviousYear, 2); ?></td>
																<?php
																// echo '<pre>';
																// var_dump($FeeDefaulterDues);
																// echo '</pre>';
																foreach ($FeeHeadList as $FeeHeadID => $FeeHeadDetails) {
																	$FeeHeadDueAmount = 0;
																	foreach ($FeeDefaulterDues as $Month => $DefaulterDetails) {
																	...
																	if ($FeeHeadDueAmount > 0) {
																		$StudentFeeHeadDueAmount += $FeeHeadDueAmount;
																	}
																}

																$TotalDue += $StudentFeeHeadDueAmount + $StudentFeeHeadDueAmountPreviousYear;
																$TotalPreviousYearDue += $StudentFeeHeadDueAmountPreviousYear;

																//var_dump($StudentFeeHeadDueAmount + $PreviousDueAmount);exit;
																?>
																<td class="text-right"><?php echo number_format($StudentFeeHeadDueAmount + $StudentFeeHeadDueAmountPreviousYear, 2); ?></td>
													...
													<?php
													        if ($LoggedUser->GetUserID() == '1000005'){
                                                                $StudentIDString = $StudentID.', '.$StudentIDString;
                                                            }
														}
														
														if ($LoggedUser->GetUserID() == '1000005'){
                                                            // error_log('$StudentIDString new: '.$StudentIDString);
                                                        }
													}
													?>
													<tr>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-lg-12" id="SMSDetails">
							<form class="form-horizontal" name="AddFeedStudentmarks" action="academic_year_wise_fee_defaulter_list.php" method="get">
								<div class="form-group">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('.DueDetails').click(function() {
				...
				DueFromDate = '<?php echo $DueFromDate; ?>';
				DueToDate = '<?php echo $DueToDate; ?>';
				...
				$.post("/xhttp_calls/get_fee_defaulter_dues_new.php", {
