...
switch ($Clean['Process']) {
	case 1:
		...
		if (isset($_POST['chkLateFee'])) {
			...
			if (isset($_POST['LateFeeType'])) {
				$Clean['LateFeeType'] = $_POST['LateFeeType'];
			}
		}
		...
		if ($Clean['LateFee'] == 1) {
			if ($AmountPaid <= 0 && $Clean['LateFeeType'] != 'Concession') {
			...
			$LateFee = 0;
			$ReaminingLateFee = $Clean['LateFeeAmount'];
			$LateFee = $Clean['LateFeeAmount'];

			//Get month wise late fee

			$LateFeesDetails = array();
			$LateFeesDetails = FeeCollection::SearchLateFee($Clean['StudentID']);

			$SaveLateFeesDetails = array();
			$TotalCharge = 0;
			foreach ($LateFeesDetails as $LateFeeID => $Details) {

				$SaveLateFeesDetails[$LateFeeID]['PaidAmount'] = $Details['Amount'];

				if ($ReaminingLateFee >= $Details['Amount'] && $Clean['LateFeeType'] != 'Due' && $Clean['LateFeeType'] != 'Concession') {
					$SaveLateFeesDetails[$LateFeeID]['Status'] = 'Paid';
					$ReaminingLateFee = $ReaminingLateFee - $Details['Amount'];
				} else if ($Clean['LateFeeType'] == 'Due') {
					$SaveLateFeesDetails[$LateFeeID]['Status'] = 'Due';
					$SaveLateFeesDetails[$LateFeeID]['PaidAmount'] = $ReaminingLateFee;
					$ReaminingLateFee = 0;
				} elseif ($Clean['LateFeeType'] == 'Concession') {
					$SaveLateFeesDetails[$LateFeeID]['Status'] = 'Concession';
					$SaveLateFeesDetails[$LateFeeID]['PaidAmount'] = $ReaminingLateFee;
					$ReaminingLateFee = 0;
				}

				$TotalCharge += $Details['Amount'];
			}

			if ($TotalCharge < (int)$Clean['LateFeeAmount']) {
				$NewRecordValidator->AttachTextError('Please enter correct late fee amount.');
				$HasErrors = true;
				break;
			}
		...
		// 			$NewRecordValidator->AttachTextError('Please select month.');
		// 			$HasErrors = true;
		// 			break;
		// 		}
		...
		FeeCollection::SaveLateFeeColletionDetails($SaveLateFeesDetails, $NewFeeCollection->GetFeeCollectionID(), $Clean['StudentID']);

		if (count($Clean['WaveOffList']) > 0) {
			if (!FeeDiscount::SetWaveOffFee($Clean['StudentID'], $Clean['WaveOffList'], $NewFeeCollection->GetCurrentTransactionID())) {
		...
		if (!FeeDiscount::SetFeeConcession($Clean['StudentID'], $Clean['ConcessionAmountList'], $NewFeeCollection->GetCurrentTransactionID())) {
			$NewRecordValidator->AttachTextError('Error in executing query.');
			$HasErrors = true;
			break;
		}
	...
	case 7:
		...
		if ($Clean['SearchType'] == 'FeeCodeWise' || $Clean['SearchType'] == 'ClassWise') {
			if ($Clean['SearchType'] == 'FeeCodeWise') {
				$AllStudents = Student::GetStudentsByContactNumber($Clean['ContactNumber']);
			} else if ($Clean['SearchType'] == 'ClassWise') {
				$AllStudents = Student::GetStudentsByStudentID($Clean['StudentID']);
			}
			...
			foreach ($MonthWiseLateDays as $MonthID => $LateDays) {
				...
				if ($TotalCharges > 0) {
					FeeCollection::SaveLateFee($Clean['StudentID'], $MonthID, $TotalCharges);
				}
			}
		}
		break;
								...
								<select class="form-control" name="drdAcademicYear" id="AcademicYearID">
									...
									if (is_array($AcademicYears) && count($AcademicYears) > 0) {
										foreach ($AcademicYears as $AcademicYearID => $AcademicYearDetails) {
											...
											echo '<option ' . ($Clean['AcademicYearID'] == $AcademicYearID ? 'selected="selected"' : '') . ' value="' . $AcademicYearID . '" ' . (($AcademicYearID == 1) ? 'disabled = "disabled"' : '') . '>' . date('Y', strtotime($AcademicYearDetails['StartDate'])) . ' - ' . date('Y', strtotime($AcademicYearDetails['EndDate'])) . '</option>';
										}
									}
									?>
								</select>
			...
			?>
				...
				<form class="form-horizontal" name="FeeCollection" id="RecordForm" action="fee_collection.php" method="post">
					<div class="row" id="RecordTable">
							...
							<table width="100%" class="table table-striped table-bordered table-hover">
								<thead>
									...
									if (isset($_SESSION['DB']) && $_SESSION['DB'] == 'addedschools_lucknowips_testing-21-22') {
										$PreviousDueAmount = FeeCollection::GetPreviousDefaulterAmount($CurrentStudentID);
									}

									$PreviousDueAmounts = 0;
									$PreviousDueAmountes = 0;
									if (isset($_SESSION['DB']) && $_SESSION['DB'] == 'addedschools_lucknowips_testing-22-23') {
										$PreviousDueAmounts = FeeCollection::GetPreviousDefaulterAmount($CurrentStudentID);
										$PreviousDueAmountes = FeeCollection::GetPrevYear3DefaulterAmount($CurrentStudentID);
										$PreviousDueAmount = $PreviousDueAmountes + $PreviousDueAmounts;
									}

									$PreviousDueAmounts = 0;
									$PreviousDueAmountes = 0;
									$PreviousDueAmountes4 = 0;
									if (isset($_SESSION['DB']) && $_SESSION['DB'] == 'addedschools_lucknowips_testing-23-24') {
										$PreviousDueAmounts = FeeCollection::GetPreviousDefaulterAmount($CurrentStudentID);
										$PreviousDueAmountes = FeeCollection::GetPrevYear3DefaulterAmount($CurrentStudentID);
										$PreviousDueAmountes4 = FeeCollection::GetPrevYear4DefaulterAmount($CurrentStudentID);
										$PreviousDueAmount = $PreviousDueAmountes + $PreviousDueAmounts + $PreviousDueAmountes4;
									}
									...
									?>
										<tr>
											...
											<th colspan="3" class="text-center"><?php echo number_format($PreviousDueAmount, 0); ?></th>
										</tr>
									<?php
									}
									?>
								...
								<tbody id="TableBody">
										...
										<td colspan="3">
											...
											foreach ($AllStudents as $StudentID => $Details) {
												foreach ($FeeMonths[$StudentID] as $MonthID => $MonthDetails) {
													...
													// 	if (@array_key_exists($MonthID, $DueFeeMonths[$StudentID][2])) {
													// 		continue;
													// 	}
													...
													// 	}
													// } else {
													// 	if (@array_key_exists($MonthID, $DueFeeMonths[$StudentID][3])) {
													// 		continue;
													// 	}
													// }
													...
													$tempAmount = array();
													$MonthDueAmount = $StudentObj->GetStudentMonthlyFeeDetails($MonthID, $tempAmount);
										...
										<td colspan="3">
											...
											foreach ($AllStudents as $StudentID => $Details) {
												...
												foreach ($DueFeeMonths[$StudentID] as $AcademicYearID => $DueFeeMonthDetail) {
													// 	 echo '<pre>';
													// 	 print_r($DueFeeMonthDetail);exit;
									...
									?>
											<tr>
												...
												<td><input type="text" class="form-control PreviousYearDueAmount" name="txtPreviousYearDueAmount" id="PreviousYearDueAmount<?php echo $StudentID; ?>" disabled="disabled" value="<?php echo (array_key_exists($StudentID, $Clean['PreviousYearDueAmount']) ? ($Clean['PreviousYearDueAmount'][$StudentID] ? $Clean['PreviousYearDueAmount'][$StudentID] : '') : ''); ?>">
													...
													<input type="hidden" name="hdnPreviousYearDueAmount[<?php echo $StudentID; ?>]" id="hdnPreviousYearDueAmount<?php echo $StudentID; ?>" value="<?php echo (array_key_exists($StudentID, $Clean['PreviousYearDueAmount']) ? ($Clean['PreviousYearDueAmount'][$StudentID] ? $Clean['PreviousYearDueAmount'][$StudentID] : '') : ''); ?>">
												</td>
									...
									<tr>
										...
										<td colspan="2"><input type="text" class="form-control" name="txtOtherFeeAmount" id="OtherFeeAmount" placeholder="Enter Other Fee Amount" disabled="disabled" value="<?php echo ($Clean['OtherFeeAmount']) ? $Clean['OtherFeeAmount'] : ''; ?>">
											<input type="hidden" name="hdnOtherFeeAmount" id="hdnOtherFeeAmount" value="<?php echo $Clean['OtherFeeAmount']; ?>">
										</td>
									</tr>
									<tr>
										...
										<td>
											...
											if ($Clean['StudentID'] > 0) {
												...
												$ShowLateFeeDetails = FeeCollection::SearchLateFee($Clean['StudentID']);

												$TotalLateCharge = 0;
												foreach ($ShowLateFeeDetails as $LateFeeID => $CurrentLateFeeDetails) {
													// foreach ($MonthWiseLateDays as $MonthID => $LateDays) {Late by ' . $LateDays . ' days
													echo '<i class="fa fa-circle text-danger" style="font-size:6px;"></i>&nbsp;&nbsp;<small class="text-danger">' . $CurrentLateFeeDetails['MonthName'] . ' = ' . number_format($CurrentLateFeeDetails['Amount'], 2) . '</small><br>';
													$TotalLateCharge += $CurrentLateFeeDetails['Amount'];
													// }
												}
										...
										<td colspan="2"><input type="text" class="form-control" name="txtLateFeeAmount" id="LateFeeAmount" placeholder="Enter Late Fee Amount" disabled="disabled" value="<?php echo $Clean['LateFeeAmount'] ? $Clean['LateFeeAmount'] : ''; ?>">
											<input type="hidden" name="hdnLateFeeAmount" id="hdnLateFeeAmount" value="<?php echo $Clean['LateFeeAmount']; ?>">
										</td>
									</tr>
									<tr style="display: none;" id="lateFeeType">
										<td>Type</td>
										<td colspan="3">
											<input type="radio" checked name="LateFeeType" id="Full Paid" value="Full Paid">&nbsp;&nbsp;Full Paid
											<input type="radio" name="LateFeeType" id="Due" value="Due">&nbsp;&nbsp;Due
											<input type="radio" name="LateFeeType" id="Concession" value="Concession">&nbsp;&nbsp;Concession
										</td>
									</tr>
									<tr>
						...
						<p style="color:red;" class="text-center">Your service will be expired on 24<sup>th</sup> December, please contact to administrator.</p>
						<div class="form-group">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#StudentName').keyup(function() {
				...
				$.post("/xhttp_calls/get_students_by_name.php", {
			...
			?>
						...
						$.post("/xhttp_calls/get_student_fees_by_months.php", {
			...
			$('.DueMonthID').change(function() {
				...
				$.post("/xhttp_calls/get_student_due_fees_by_months.php", {
					SelectedMonthID: MonthID,
					SelectedStudentID: StudentID,
					SelectedAcademicYearID: AcademicYearID
				}, function(data) {
			...
			$('#LateFee').change(function() {
				if ($(this).is(':checked')) {
					...
					$("#lateFeeType").attr("style", "display: 'block'");
					...
					$("#lateFeeType").attr("style", "display:none");
					if ($('#LateFeeAmount').val() != '') {
						var AmountPayable = parseInt($('#AmountPayable').val());
						var LateFeeAmount = parseInt($('#LateFeeAmount').val());
