...
$Clean['DiscountForAYID'] = 0;
$Clean['DiscountINAYID'] = 0;
$Clean['ClassID'] = 0;
...
$Clean['DiscountFromDate'] = '';
$Clean['DiscountToDate'] = '';
$Clean['StudentName'] = '';
$Clean['MobileNumber'] = '';
...
switch ($Clean['Process']) {
    ...
    case 7:
        ...
        if (isset($_GET['drdDiscountForAYID'])) {
            $Clean['DiscountForAYID'] = (int) $_GET['drdDiscountForAYID'];
        } else if (isset($_GET['DiscountForAYID'])) {
            $Clean['DiscountForAYID'] = (int) $_GET['DiscountForAYID'];
        }

        if (isset($_GET['drdDiscountINAYID'])) {
            $Clean['DiscountINAYID'] = (int) $_GET['drdDiscountINAYID'];
        } else if (isset($_GET['DiscountINAYID'])) {
            $Clean['DiscountINAYID'] = (int) $_GET['DiscountINAYID'];
        }
        ...
        if (isset($_GET['optStatus'])) {
            $Clean['Status'] = (int) $_GET['optStatus'];
        } elseif (isset($_GET['Status'])) {
            $Clean['Status'] = (int) $_GET['Status'];
        }

        if (isset($_GET['txtDiscountFromDate'])) {
            $Clean['DiscountFromDate'] = strip_tags(trim($_GET['txtDiscountFromDate']));
        } elseif (isset($_GET['DiscountFromDate'])) {
            $Clean['DiscountFromDate'] = strip_tags(trim($_GET['DiscountFromDate']));
        }

        if (isset($_GET['txtDiscountToDate'])) {
            $Clean['DiscountToDate'] = strip_tags(trim($_GET['txtDiscountToDate']));
        } elseif (isset($_GET['DiscountToDate'])) {
            $Clean['DiscountToDate'] = strip_tags(trim($_GET['DiscountToDate']));
        }
        ...
        if ($Clean['DiscountFromDate'] != '') {
            $SearchValidator->ValidateDate($Clean['DiscountFromDate'], 'Please enter valid discount from date.');
            $SearchValidator->ValidateDate($Clean['DiscountToDate'], 'Please enter valid discount to date.');
        }
        ...
        $Filters['DiscountForAYID'] = $Clean['DiscountForAYID'];
        $Filters['DiscountINAYID'] = $Clean['DiscountINAYID'];
        ...
        if ($Clean['Status'] == 1 || $Clean['Status'] == 2) {
            if ($Clean['Status'] == 1) {
        ...
        if ($Clean['DiscountFromDate'] != '') {
            $Filters['DiscountFromDate'] = date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['DiscountFromDate']))));
        }

        if ($Clean['DiscountToDate'] != '') {
            $Filters['DiscountToDate'] = date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['DiscountToDate']))));
        }
...
?>
<title>Student Discount Details New</title>
<!-- DataTables CSS -->
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Student Discount Details New</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
                    ...
                    <div id="collapseOne" class="panel-collapse collapse in">
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="DiscountForAYID" class="col-lg-2 control-label">Discount For <span style="cursor: pointer;" title="Academic Year">A.Y</span></label>
                                <div class="col-lg-3">

                                    <select class="form-control" name="drdDiscountForAYID" id="DiscountForAYID" onchange="return disableStudentdrd();">
                                        <option value="0">All</option>
                                        <?php
                                        if (is_array($AcademicYears) && count($AcademicYears) > 0) {
                                            foreach ($AcademicYears as $AcademicYearID => $AcademicYearDetails) {
                                                echo '<option ' . ($Clean['DiscountForAYID'] == $AcademicYearID ? 'selected="selected"' : '') . ' value="' . $AcademicYearID . '" >' . date('Y', strtotime($AcademicYearDetails['StartDate'])) . ' - ' . date('Y', strtotime($AcademicYearDetails['EndDate'])) . '</option>';
                                            }
                                        }
                                        ?>
                                    </select>
                            ...
                            <div class="form-group">
                                <label for="DiscountINAYID" class="col-lg-2 control-label">Discount In <span style="cursor: pointer;" title="Academic Year">A.Y</span></label>
                                <div class="col-lg-3">
                                    <select class="form-control" name="drdDiscountINAYID" id="DiscountINAYID" onchange="return disableStudentdrd();">
                                        <?php
                                        if (is_array($AcademicYears) && count($AcademicYears) > 0) {
                                            foreach ($AcademicYears as $AcademicYearID => $AcademicYearDetails) {
                                                echo '<option ' . ($Clean['DiscountINAYID'] == $AcademicYearID ? 'selected="selected"' : '') . ' value="' . $AcademicYearID . '" >' . date('Y', strtotime($AcademicYearDetails['StartDate'])) . ' - ' . date('Y', strtotime($AcademicYearDetails['EndDate'])) . '</option>';
                                            }
                                        }
                                        ?>
                                    </select>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-lg-6">
                                    <label>
                                        <input class="custom-radio" type="radio" name="optStatus" value="0" <?php echo ($Clean['Status'] == 0) ? 'checked="checked"' : ''; ?>>&nbsp;&nbsp;All
                                    </label>
                                    <label>
                                        <input class="custom-radio" type="radio" name="optStatus" value="1" <?php echo ($Clean['Status'] == 1) ? 'checked="checked"' : ''; ?>>&nbsp;&nbsp;Active
                                    </label>
                                    <label>
                                        <input class="custom-radio" type="radio" name="optStatus" value="2" <?php echo ($Clean['Status'] == 2) ? 'checked="checked"' : ''; ?>>&nbsp;&nbsp;InActive
                                    </label>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="DiscountFromDate" class="col-lg-3 control-label">Discount Between</label>
                                <div class="col-lg-3">
                                    <input class="form-control select-date" type="text" maxlength="10" id="DiscountFromDate" name="txtDiscountFromDate" value="<?php echo $Clean['DiscountFromDate']; ?>" />
                                </div>
                                <label for="DiscountToDate" class="col-lg-1 control-label">to</label>
                                <div class="col-lg-3">
                                    <input class="form-control select-date" type="text" maxlength="10" id="DiscountToDate" name="txtDiscountToDate" value="<?php echo $Clean['DiscountToDate']; ?>" />
                                </div>
                            </div>
                            <div class="form-group">
            ...
            if ($Clean['Process'] == 7 && $HasErrors == false) {
                ...
                if ($Clean['DiscountForAYID'] != 0) {
                    $ReportHeaderText .= ' Discount For A.Y: ' . date('Y', strtotime($AcademicYears[$Clean['DiscountForAYID']]['StartDate'])) . ' - ' . date('Y', strtotime($AcademicYears[$Clean['DiscountForAYID']]['EndDate'])) . ',';
                }

                if ($Clean['DiscountINAYID'] != 0) {
                    $ReportHeaderText .= ' Discount IN A.Y: ' . date('Y', strtotime($AcademicYears[$Clean['DiscountINAYID']]['StartDate'])) . ' - ' . date('Y', strtotime($AcademicYears[$Clean['DiscountINAYID']]['EndDate'])) . ',';
                }
                            ...
                            <div class="panel-body">
                                <div>
                                    <div class="row">
                                        <div class="col-lg-6">
                                            ...
                                            if ($TotalPages > 1) {
                                                $AllParameters = array(
                                                    'Process' => '7',
                                                    'DiscountForAYID' => $Clean['DiscountForAYID'],
                                                    'DiscountINAYID' => $Clean['DiscountINAYID'],
                                                    'ClassID' => $Clean['ClassID'],
                                                    'ClassSectionID' => $Clean['ClassSectionID'],
                                                    'StudentID' => $Clean['StudentID'],
                                                    'StudentName' => $Clean['StudentName'],
                                                    'MobileNumber' => $Clean['MobileNumber'],
                                                    'DiscountType' => $Clean['DiscountType'],
                                                    'Status' => $Clean['Status'],
                                                    'DiscountFromDate' => $Clean['DiscountFromDate'],
                                                    'DiscountToDate' => $Clean['DiscountToDate'],
                                    ...
                                    <div class="row" id="RecordTableHeading">
                                        <div class="col-lg-12">
                                            <div class="report-heading-container"><strong>Student Discount Details New on <?php echo date('d-m-Y h:i A') . $ReportHeaderText; ?></strong></div>
                                        </div>
                                    </div>
                                                ...
                                                <tbody>
                                                    ...
                                                    ?>
                                                            <tr>
                                                                ...
                                                                <td><?php echo $Details['TransactionDate']; ?></td>
                                                                <td><?php echo $Details['DiscountDate']; ?></td>
    ...
    <script type="text/javascript">
        ...
        function disableStudentdrd() {
            if ($('#DiscountForAYID').val() == 1 || $('#DiscountINAYID').val() == 1) {
