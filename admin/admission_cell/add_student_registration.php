...
switch ($Clean['Process']) {
    case 1:
        ...
        if ($Clean['ExamID'] > 0) {
            ...
            if ($TemporaryUser) {
                $NewExamRegistration->SetCreateUserID(1000001);
            } else {
        ...
        try {
            $NewSMSTemplate = new SMSTemplate(0, 'StudentRegistration');

            if ($NewSMSTemplate->GetIsActive()) {
                $MSGContent = $NewSMSTemplate->GetSMSTemplate();

                $MSGContent = str_replace('{StudentFirstName}', $Clean['FirstName'], $MSGContent);
                $MSGContent = str_replace('{StudentLastName}', $Clean['LastName'], $MSGContent);
                $MSGContent = str_replace('{FatherName}', $Clean['FatherName'], $MSGContent);
                $MSGContent = str_replace('{MotherName}', $Clean['MotherName'], $MSGContent);
                $MSGContent = str_replace('{Class}', $ClassList[$Clean['ClassID']], $MSGContent);
                $MSGContent = str_replace('{Section}', '', $MSGContent);
                if ($Clean['ExamDate'] != '') {
                    $MSGContent = str_replace('{EntranceExamDateTime}', date('Y-m-d', strtotime(DateProcessing::ToggleDateDayAndMonth(($Clean['ExamDate'])))) . ' ' . $Clean['ExamTime'], $MSGContent);
                }

                $NewSMSQueue = new SMSQueue();

                $NewSMSQueue->SetPhoneNumber($Clean['MobileNumber']);
                $NewSMSQueue->SetSMSMessage($MSGContent);

                if ($TemporaryUser) {
                    $NewSMSQueue->SetCreateUserID(1000001);
                } else {
            ...
            <form class="form-horizontal" name="AddRegistrationt" action="add_student_registration.php" method="post">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            <label for="Class" class="col-lg-2 control-label">Admission in Class</label>
                            <div class="col-lg-4">
