...
{   
    if ($FeeTransactionID > 0)
    {
        ...
        $excelWriter->setActiveSheetIndex(0)
            ...
            ->setCellValueExplicit('C' . $index, $FeeTransactionID, PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('D' . $index, $StudentName, PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('E' . $index, $Details['ClassName'] . ' (' . $Details['SectionName'] . ')', PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('F' . $index, $Details['FatherName'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('G' . $index, $Details['TransactionAmount'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('H' . $index, date('d/m/Y', strtotime($Details['FeeDate'])), PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit('I' . $index, $PaymentMode, PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit('J' . $index, $Details['Description'], PHPExcel_Cell_DataType::TYPE_STRING);
...
$excelWriter->setActiveSheetIndex(0)
			...
			->setCellValueExplicit('F' . $index, number_format($TotalAmountPaid, 2), PHPExcel_Cell_DataType::TYPE_STRING)
			...
			->setCellValueExplicit('J' . $index, '', PHPExcel_Cell_DataType::TYPE_STRING);
			
    $excelWriter->getActiveSheet()->getStyle('A' . $index .  ':J' . $index)->getFont()->setBold(true);
            $excelWriter->getActiveSheet()->getStyle('A' . $index . ':J' . $index)->getFont()->getColor()->setARGB('FFFFFFFF');
