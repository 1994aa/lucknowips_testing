...
$excelWriter->setActiveSheetIndex(0)
        ...
        ->setCellValue('K1', 'FContactNo')
        ->setCellValue('L1', 'MContactNo')
        ->setCellValue('M1', 'Address')
        ->setCellValue('N1', 'Admission Date')
        ->setCellValue('O1', 'User Name')
        ->setCellValue('P1', 'Password');

$excelWriter->getActiveSheet()->getStyle('A1:P1')->getFont()->setBold(true);
$excelWriter->getActiveSheet()->getStyle('A1:P1')->getFont()->getColor()->setARGB('FFFFFFFF');
$excelWriter->getActiveSheet()->getStyle('A1:P1')->applyFromArray(
        array('fill' => array(
...
foreach($AllStudents as $StudentID => $StudentDetails)
{
    ++$index;
    $Password = '';
    $Password = strtoupper(substr($StudentDetails['UserName'], 0, 4)) . substr($StudentDetails['Dob'], 0, 4) ;
    ...
    $excelWriter->setActiveSheetIndex(0)
			...
			->setCellValueExplicit('J' . $index, $StudentDetails['Category'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('K' . $index, $StudentDetails['FatherMobileNumber'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('L' . $index, $StudentDetails['MotherMobileNumber'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('M' . $index, $StudentDetails['Address'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('N' . $index, date('d/m/Y', strtotime($StudentDetails['AdmissionDate'])), PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('O' . $index, $StudentDetails['UserName'], PHPExcel_Cell_DataType::TYPE_STRING)
            ->setCellValueExplicit('P' . $index, $Password, PHPExcel_Cell_DataType::TYPE_STRING);

    if ($index % 2 == 0)
    {
        $excelWriter->getActiveSheet()->getStyle('A' . $index . ':P' . $index)->applyFromArray(
                array('fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
...
$excelWriter->getActiveSheet()->getStyle('A1:P' . $index)->applyFromArray(
        array(
            'borders' => array(
