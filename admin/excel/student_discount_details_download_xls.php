...
$excelWriter->setActiveSheetIndex(0)
        ...
        ->setCellValue('F1', 'Fee Head')
        ->setCellValue('G1', 'Total Amt')
        ->setCellValue('H1', $Clean['DiscountType'],'Amt');
    ...
    {   
        if ($FeeDiscountID > 0)
        {
            ...
            $excelWriter->setActiveSheetIndex(0)
                ...
                ->setCellValueExplicit('E' . $index, $Details['MonthName'], PHPExcel_Cell_DataType::TYPE_STRING)
                ->setCellValueExplicit('F' . $index, $Details['FeeHead'], PHPExcel_Cell_DataType::TYPE_STRING)
                ->setCellValueExplicit('G' . $index, number_format($Details['TotalAmount'], 2) , PHPExcel_Cell_DataType::TYPE_STRING)
                ->setCellValueExplicit('H' . $index, number_format($DiscountTypeAmount, 2) , PHPExcel_Cell_DataType::TYPE_STRING);
...
$excelWriter->setActiveSheetIndex(0)
			...
			->setCellValueExplicit('F' . $index, 'Grand Total', PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit('G' . $index, number_format($TotalAmount, 2), PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit('H' . $index, number_format($TotalDiscountAmount, 2), PHPExcel_Cell_DataType::TYPE_STRING);
			
    $excelWriter->getActiveSheet()->getStyle('A' . $index .  ':H' . $index)->getFont()->setBold(true);
            $excelWriter->getActiveSheet()->getStyle('A' . $index . ':H' . $index)->getFont()->getColor()->setARGB('FFFFFFFF');
