...
class Helpers
{
	...
	static function GetStudentDOB()
	{
		...
		try {
			...
			$RSSearch = $DBConnObject->Prepare('SELECT asd.firstName, asd.lastName, asd.dob, a.classSectionID, ac.classID, ac.className, asd.studentID
														FROM asa_students a
														INNER JOIN asa_student_details asd ON asd.studentID = a.studentID
														INNER JOIN asa_class_sections acs ON acs.classSectionID = a.classSectionID
														INNER JOIN asa_classes ac ON acs.classID = ac.classID
														WHERE asd.dob = CURDATE();');
			$RSSearch->Execute();

			if ($RSSearch->Result->num_rows <= 0) {
				return $StudentDOBList;
			}

			while ($SearchRow = $RSSearch->FetchRow()) {
				$StudentDOBList[$SearchRow->studentID]['FirstName'] = $SearchRow->firstName;
				$StudentDOBList[$SearchRow->studentID]['LastName'] = $SearchRow->lastName;
				$StudentDOBList[$SearchRow->studentID]['DOB'] = $SearchRow->dob;
				$StudentDOBList[$SearchRow->studentID]['ClassName'] = $SearchRow->className;
			}

			return $StudentDOBList;
		} catch (ApplicationDBException $e) {
			error_log('DEBUG: ApplicationDBException at Helpers::GetStudentDOB(). Stack Trace: ' . $e->getTraceAsString());
			return $StudentDOBList;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at Helpers::GetStudentDOB(). Stack Trace: ' . $e->getTraceAsString());
			return $StudentDOBList;
		}
	}

	static function GetStudentAbsentList($num_of_days = 3)
	{
		...
		try {
			...
			//get dates for absent
			$RSSearchDates = $DBConnObject->Prepare('SELECT * FROM asa_class_attendence
													WHERE attendenceDate < :|1
													GROUP BY attendenceDate 
													ORDER BY attendenceDate DESC LIMIT ' . $num_of_days . ';');
			$RSSearchDates->Execute($current_date);


			if ($RSSearchDates->Result->num_rows <= 0) {
				return $dates;
			}

			while ($SearchDateRow = $RSSearchDates->FetchRow()) {
				$dates[] = $SearchDateRow->attendenceDate;
			}

			foreach ($dates as $date) {
				$RSSearch = $DBConnObject->Prepare('SELECT * FROM asa_class_attendence WHERE attendenceDate = :|1;');
				$RSSearch->Execute($date);

				while ($SearchRow = $RSSearch->FetchRow()) {
					$RSSearchOne = $DBConnObject->Prepare('SELECT asd.firstName, asd.lastName, a.classSectionID, ac.classID, ac.className,
																 asd.studentID
																FROM asa_class_attendence_details acad
																INNER JOIN asa_students a ON a.studentID = acad.studentID
																INNER JOIN asa_student_details asd ON asd.studentID = a.studentID
																INNER JOIN asa_class_sections acs ON acs.classSectionID = a.classSectionID
																INNER JOIN asa_classes ac ON acs.classID = ac.classID 
																WHERE classAttendenceID = :|1	AND attendenceStatus = "Absent";');

					$RSSearchOne->Execute($SearchRow->classAttendenceID);

					if ($RSSearchOne->Result->num_rows <= 0) {
						continue;
					}

					if (count($absenties) > 0) {
						while ($SearchRowOne = $RSSearchOne->FetchRow()) {
							if (array_key_exists($SearchRowOne->studentID, $absenties)) {
								$absenties[$SearchRowOne->studentID]['Counter'] += 1;
							}
						}
					} else {
						while ($SearchRowOne = $RSSearchOne->FetchRow()) {
							$absenties[$SearchRowOne->studentID]['StudentID'] = $SearchRowOne->studentID;
							$absenties[$SearchRowOne->studentID]['FirstName'] = $SearchRowOne->firstName;
							$absenties[$SearchRowOne->studentID]['LastName'] = $SearchRowOne->lastName;
							$absenties[$SearchRowOne->studentID]['ClassName'] = $SearchRowOne->className;
							$absenties[$SearchRowOne->studentID]['Counter'] = 1;
						}
					}
				}
			...
			foreach ($absenties as $studentID => $details) {
				if ($details['Counter'] != $num_of_days) {
					unset($absenties[$studentID]);
				}
			}

			return $absenties;
		} catch (ApplicationDBException $e) {
			error_log('DEBUG: ApplicationDBException at Helpers::GetStudentAbsentList(). Stack Trace: ' . $e->getTraceAsString());
			return $absenties;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at Helpers::GetStudentAbsentList(). Stack Trace: ' . $e->getTraceAsString());
			return $absenties;
		}
	}
	...
	static function UniqueIDExists($UniqueID)
	{
		try {
			...
			$RSSearch = $DBConnObject->Prepare('SELECT COUNT(*) AS totalRecords FROM addedschools_central.all_user_accounts WHERE userName = :|1;');
	...
	static function SaveUniqueID($UniqueID, $AadharNumber)
	{
		try {
			...
			$RSSave = $DBConnObject->Prepare('INSERT INTO addedschools_central.all_user_accounts (userName, aadharNumber, schoolName) VALUES (:|1, :|2, :|3);');
			$RSSave->Execute($UniqueID, $AadharNumber, 'lucknowips');
					...
					if ($RSSearchFeeStructureID->Result->num_rows > 0) {
						...
						if ($RSSearchFeeStructureDetails->Result->num_rows > 0) {
							while ($SearchRow = $RSSearchFeeStructureDetails->FetchRow()) {
								...
								if ($AmountPayable > 0) {
									$RSSaveStudentFeeStrucutre = $DBConnObject->Prepare('INSERT INTO afm_student_fee_structure (academicYearID, feeStructureDetailID, studentID, amountPayable)
																							VALUES(:|1, :|2, :|3, :|4);');

									$RSSaveStudentFeeStrucutre->Execute($NextAcademicYearID, $SearchRow->feeStructureDetailID, $StudentID, $AmountPayable);
								}
							}
						}
					}
