...
class SMSQueue
{
	...
	static function SMSInQueue()
	{
		...
		try {
			...
			$RSTotalSMS = $DBConnObject->Prepare('SELECT COUNT(smsQueueID) AS totalSMS FROM sms_queue_old WHERE smsStatus = "Not Sent";');
			$RSTotalSMS->Execute();

			if ($RSTotalSMS->Result->num_rows <= 0) {
				return $SMSInQueue;
			}

			$SMSInQueue['TotalSMS'] = $RSTotalSMS->FetchRow()->totalSMS;

			$RSTotalSMSLength = $DBConnObject->Prepare('SELECT SUM(CEIL((LENGTH(smsMessage))/160)) AS totalSMSLength FROM sms_queue_old WHERE smsStatus = "Not Sent";');
			$RSTotalSMSLength->Execute();

			if ($RSTotalSMSLength->Result->num_rows <= 0) {
				return $SMSInQueue;
			}
	...
	static function GetSMSReport(&$TotalRecords = 0, &$TotalSMSUsed = 0, $GetTotalsOnly = false, $Filters = array(), $Start = 0, $Limit = 100)
	{
		...
		try {
			...
			if ($GetTotalsOnly) {
				...
				$RSTotals = $DBConnObject->Prepare('SELECT COUNT(*) AS totalRecords FROM sms_queue_old sq' . $QueryString . ';');
				$RSTotals->Execute();

				if ($RSTotals->Result->num_rows <= 0) {
					return $SMSList;
				}

				$TotalRecords = $RSTotals->FetchRow()->totalRecords;

				$RSTotalSMSUsed = $DBConnObject->Prepare('SELECT SUM(smsUsed) AS totalSMSUsed FROM sms_queue_old sq' . $QueryString . ';');
	...
	static function UpdateSMSSent($SMSQueueID)
	{
		try {
			...
			$RSUpdateSMSQueue = $DBConnObject->Prepare('UPDATE sms_queue_old SET smsStatus = \'Pending\', sentDateTime = NOW() WHERE smsQueueID = :|1 LIMIT 1;');
	...
	private function SaveDetails()
	{
		if ($this->SMSQueueID == 0) {
			...
			$RSSave = $this->DBObject->Prepare('INSERT INTO sms_queue_old (phoneNumber, smsMessage, smsUsed, createUserID, createDate)
														VALUES (:|1, :|2, :|3, :|4, NOW());');
	...
	{
		$RSSMSQueue = $this->DBObject->Prepare('SELECT * FROM sms_queue_old WHERE smsQueueID = :|1 LIMIT 1;');
