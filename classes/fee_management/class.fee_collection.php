...
class FeeCollection
{
	...
	static function GetUpdateDB()
	{
		try {
			...
			$RSUpdate = $DBConnObject->Prepare('UPDATE afm_fee_collection_details SET academicYearID = 2 WHERE academicYearID = 0 ;');
			$RSUpdate->Execute();

			$RSUpdate = $DBConnObject->Prepare("UPDATE afm_fee_transactions SET academicYearID = '2' WHERE academicYearID = 0;");
			$RSUpdate->Execute();

			$RSUpdate = $DBConnObject->Prepare('UPDATE afm_fee_discounts fd
                                                    INNER JOIN afm_fee_structure_details fsd ON fd.feeStructureDetailID = fsd.feeStructureDetailID
	...
	static function GetPrevYearDefaulterAmount($StudentID, $Year, $DueToDate = 0)
	{
		$Amount = 0;
		try {
			...
			$query_condition = '';
			$dis_query_condition = '';
			$waveoff_query_condition = '';
			if (!empty($DueToDate)) {
				$query_condition = "AND (fc.feeDate <= '$DueToDate')";
				$dis_query_condition = "AND (fd.discountDateTime IS NOT NULL AND DATE(fd.discountDateTime) <= '$DueToDate')";
				$waveoff_query_condition = "AND (fd.transactionDateTime IS NOT NULL AND DATE(fd.transactionDateTime) <= '$DueToDate')";
			}
			if ($Year == 2) {
			...
			$RSFeeDefaulterDues = $DBConnObject->Prepare('SELECT afh.feeHead, afh.feeHeadID, afsd.feeAmount, aaym.monthName, 
														...
														(
															...
															AND sfss.academicYearID = ' . $Year . ' AND fcd.academicYearID != 0 ' . $query_condition . '
															AND fsds.academicYearID = ' . $Year . ') AS totalAmountPaid,
														(
															SELECT IFNULL(SUM(fd.calculatedDiscountAmount - (fd.concessionAmount + fd.waveOffAmount)), 0)
															...
															AND fsds.academicYearID = ' . $Year . '  ' . $dis_query_condition . '
														) As totalDiscountAmount, 
														(
															SELECT IFNULL(SUM(fd.concessionAmount + fd.waveOffAmount), 0)
															FROM `addedschools_lucknowips_testing`.afm_fee_discounts fd
															INNER JOIN `addedschools_lucknowips_testing`.afm_fee_structure_details fsds ON fsds.feeStructureDetailID = fd.feeStructureDetailID
															WHERE fd.studentID = ' . $StudentID . ' AND fsds.feeStructureDetailID = afsd.feeStructureDetailID
															AND fsds.academicYearID = ' . $Year . ' ' . $waveoff_query_condition . '
														) As totalWaveoffAmount
			...
			while ($SearchRow = $RSFeeDefaulterDues->FetchRow()) {
				$DueAmount = $SearchRow->amountPayable - ($SearchRow->totalAmountPaid + $SearchRow->totalDiscountAmount + $SearchRow->totalWaveoffAmount);
	...
	static function GetPrevYear3DefaulterAmount($StudentID, $Year = 0, $DueToDate = 0)
	{
		$Amount = 0;
		try {
			...
			$Year = 2;
			$query_condition = '';
			$disc_query_condition = '';
			$waveOff_query_condition = '';
			if (!empty($DueToDate)) {
				$query_condition = "AND (fc.feeDate <= '$DueToDate')";
				$disc_query_condition = "AND (fd.discountDateTime IS NOT NULL AND DATE(fd.discountDateTime) <= '$DueToDate')";
				$waveOff_query_condition = "AND (fd.transactionDateTime IS NOT NULL AND DATE(fd.transactionDateTime) <= '$DueToDate')";
			}
			...
			$RSFeeDefaulterDues = $DBConnObject->Prepare('SELECT afh.feeHead, afh.feeHeadID, afsd.feeAmount, aaym.monthName, 
														...
														(
															SELECT IFNULL(SUM(fcd.amountPaid), 0)
															FROM `addedschools_lucknowips_testing-21-22`.afm_student_fee_structure sfss
															INNER JOIN `addedschools_lucknowips_testing-21-22`.afm_fee_collection fc ON (sfss.studentID = fc.studentID)
															INNER JOIN `addedschools_lucknowips_testing-21-22`.afm_fee_collection_details fcd ON (fc.feeCollectionID = fcd.feeCollectionID AND sfss.studentFeeStructureID = fcd.studentFeeStructureID)
															INNER JOIN `addedschools_lucknowips_testing-21-22`.afm_fee_structure_details fsds ON (sfss.feeStructureDetailID = fsds.feeStructureDetailID)
															WHERE sfss.studentID = ' . $StudentID . ' AND sfss.studentFeeStructureID = asfs.studentFeeStructureID
															AND sfss.academicYearID = ' . $Year . ' AND fcd.academicYearID != 0 ' . $query_condition . '
															AND fsds.academicYearID = ' . $Year . ') AS totalAmountPaid,
														(
															SELECT IFNULL(SUM(fd.calculatedDiscountAmount - (fd.concessionAmount + fd.waveOffAmount)), 0)
															FROM `addedschools_lucknowips_testing-21-22`.afm_fee_discounts fd
															INNER JOIN `addedschools_lucknowips_testing-21-22`.afm_fee_structure_details fsds ON fsds.feeStructureDetailID = fd.feeStructureDetailID
															WHERE fd.studentID = ' . $StudentID . ' AND fsds.feeStructureDetailID = afsd.feeStructureDetailID
															AND fsds.academicYearID = ' . $Year . ' ' . $disc_query_condition . '
														) As totalDiscountAmount, 
														(
															SELECT IFNULL(SUM(fd.concessionAmount + fd.waveOffAmount), 0)
															FROM `addedschools_lucknowips_testing-21-22`.afm_fee_discounts fd
														...
														FROM `addedschools_lucknowips_testing-21-22`.afm_student_fee_structure asfs
														INNER JOIN `addedschools_lucknowips_testing-21-22`.afm_fee_structure_details afsd ON asfs.feeStructureDetailID = afsd.feeStructureDetailID
														INNER JOIN `addedschools_lucknowips_testing-21-22`.afm_fee_heads afh ON afh.feeHeadID = afsd.feeHeadID
														INNER JOIN `addedschools_lucknowips_testing-21-22`.asa_academic_year_months aaym ON aaym.academicYearMonthID = afsd.academicYearMonthID
														WHERE asfs.studentID = :|1 ' . $Condition . ' AND asfs.academicYearID = ' . $Year . '
														GROUP BY afsd.feeHeadID, afsd.academicYearMonthID
														ORDER BY aaym.feePriority;');
			...
			while ($SearchRow = $RSFeeDefaulterDues->FetchRow()) {
				$DueAmount = $SearchRow->amountPayable - ($SearchRow->totalAmountPaid + $SearchRow->totalDiscountAmount + $SearchRow->totalWaveoffAmount);
				// $DueAmount = $SearchRow->amountPayable - ($SearchRow->totalAmountPaid);
			...
			error_log('DEBUG: ApplicationDBException at FeeCollection::GetPrevYear3DefaulterAmount(). Stack Trace: ' . $e->getTraceAsString());
			return $Amount;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at FeeCollection::GetPrevYear3DefaulterAmount(). Stack Trace: ' . $e->getTraceAsString());
			return $Amount;
		}
	}

	static function GetPrevYear4DefaulterAmount($StudentID, $Year = 0, $DueToDate = 0)
	{
		$Amount = 0;
		try {
			...
			$RSFeeDefaulterDues = $DBConnObject->Prepare('SELECT afh.feeHead, afh.feeHeadID, afsd.feeAmount, aaym.monthName, 
														...
														(
															SELECT IFNULL(SUM(fcd.amountPaid), 0)
															FROM `addedschools_lucknowips_testing-22-23`.afm_student_fee_structure sfss
															INNER JOIN `addedschools_lucknowips_testing-22-23`.afm_fee_collection fc ON (sfss.studentID = fc.studentID)
															INNER JOIN `addedschools_lucknowips_testing-22-23`.afm_fee_collection_details fcd ON (fc.feeCollectionID = fcd.feeCollectionID AND sfss.studentFeeStructureID = fcd.studentFeeStructureID)
															INNER JOIN `addedschools_lucknowips_testing-22-23`.afm_fee_structure_details fsds ON (sfss.feeStructureDetailID = fsds.feeStructureDetailID)
															WHERE sfss.studentID = ' . $StudentID . ' AND sfss.studentFeeStructureID = asfs.studentFeeStructureID
															AND sfss.academicYearID = ' . $Year . ' AND fcd.academicYearID != 0 ' . $query_condition . '
															AND fsds.academicYearID = ' . $Year . ') AS totalAmountPaid,
														(
															SELECT IFNULL(SUM(fd.calculatedDiscountAmount - (fd.concessionAmount + fd.waveOffAmount)), 0)
															FROM `addedschools_lucknowips_testing-22-23`.afm_fee_discounts fd
															INNER JOIN `addedschools_lucknowips_testing-22-23`.afm_fee_structure_details fsds ON fsds.feeStructureDetailID = fd.feeStructureDetailID
															WHERE fd.studentID = ' . $StudentID . ' AND fsds.feeStructureDetailID = afsd.feeStructureDetailID
															AND fsds.academicYearID = ' . $Year . ' ' . $disc_query_condition . '
														) As totalDiscountAmount, 
														(
														...
														FROM `addedschools_lucknowips_testing-22-23`.afm_student_fee_structure asfs
														INNER JOIN `addedschools_lucknowips_testing-22-23`.afm_fee_structure_details afsd ON asfs.feeStructureDetailID = afsd.feeStructureDetailID
														INNER JOIN `addedschools_lucknowips_testing-22-23`.afm_fee_heads afh ON afh.feeHeadID = afsd.feeHeadID
														INNER JOIN `addedschools_lucknowips_testing-22-23`.asa_academic_year_months aaym ON aaym.academicYearMonthID = afsd.academicYearMonthID
														WHERE asfs.studentID = :|1 ' . $Condition . ' AND asfs.academicYearID = ' . $Year . '
														GROUP BY afsd.feeHeadID, afsd.academicYearMonthID
														ORDER BY aaym.feePriority;');
			...
			error_log('DEBUG: ApplicationDBException at FeeCollection::GetPrevYear4DefaulterAmount(). Stack Trace: ' . $e->getTraceAsString());
			return $Amount;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at FeeCollection::GetPrevYear4DefaulterAmount(). Stack Trace: ' . $e->getTraceAsString());
			return $Amount;
		}
	}

	static function GetPreviousDefaulterAmount($StudentID, $Year = 0, $DueToDate = 0)
	{
		$PrevTableAmount = self::GetPrevTableAmount($StudentID);
		$PrevYear1Amount = self::GetPrevYearDefaulterAmount($StudentID, 2, $DueToDate);
		$PrevYear2Amount = self::GetPrevYearDefaulterAmount($StudentID, 1, $DueToDate);
		...
		try {
			...
			while ($SearchRow = $RSSearchStudent->FetchRow()) {
				...
				if ($TotalPreviousAmount > 0) {
					$RSCount = $DBConnObject->Prepare('SELECT COUNT(previousYearFeeDetailID) AS totalRecords FROM afm_previous_year_fee_details_21_22 WHERE academicYearID = 2 AND studentID = :|1 AND payableAmount = :|2 LIMIT 1;');
					$RSCount->Execute($SearchRow->studentID, $TotalPreviousAmount);
		...
		try {
			...
			if (count($Filters) > 0) {
				...
				if (!empty($Filters['AdmissionStatus']) && $Filters['AdmissionStatus'] == 'New') {
					$Conditions[] = 'afg.feeGroup = "NEW STUDENT"';
				}

				if (!empty($Filters['AdmissionStatus']) && $Filters['AdmissionStatus'] == 'Old') {
					$Conditions[] = 'afg.feeGroup = "OLD STUDENT"';
				}
			...
			if ($GetTotalsOnly) {
				$RSTotal = $DBConnObject->Prepare('SELECT COUNT(DISTINCT aft.feeTransactionID) AS totalRecords 
													...
													INNER JOIN afm_fee_groups afg ON afg.feeGroupID = afs.feeGroupID
			...
			$RSSearch = $DBConnObject->Prepare('SELECT aft.feeTransactionID, aft.description, aft.createDate, aft.transactionAmount AS totalTransactionAmount, afc.feeDate, 
			                                            apd.fatherFirstName, apd.fatherLastName, u.userName AS createUserName, apd.fatherMobileNumber, apd.motherMobileNumber,
			                                            asd.mobileNumber, ac.className, asm.sectionName, ass.enrollmentID
												FROM afm_fee_transactions aft
												INNER JOIN afm_fee_payment_mode_details afpmd ON afpmd.feeTransactionID = aft.feeTransactionID
												INNER JOIN afm_fee_collection afc ON afc.feeTransactionID = aft.feeTransactionID
			...
			while ($SearchRow = $RSSearch->FetchRow()) {
				$FeeTransactionDetails[$SearchRow->feeTransactionID]['EnrollmentID'] = $SearchRow->enrollmentID;
				...
				$FeeTransactionDetails[$SearchRow->feeTransactionID]['ClassName'] = $SearchRow->className;
				$FeeTransactionDetails[$SearchRow->feeTransactionID]['SectionName'] = $SearchRow->sectionName;
	...
	static function SearchFeeCollectionDetailsNew(&$TotalRecords = 0, $GetTotalsOnly = false, $Filters = array(), $Start = 0, $Limit = 100)
	{
		...
		try {
			...
			if (count($Filters) > 0) {
				if ($_SESSION['DB'] != 'addedschools_lucknowips_testing-21-22' && $_SESSION['DB'] != 'addedschools_lucknowips_testing-22-23' && $_SESSION['DB'] != 'addedschools_lucknowips_testing-23-24') {
					$Conditions[] = 'asfs.academicYearID < 4';
				}
				...
				} else if ($Filters['CollectionAcademicYearID'] == 4 && $_SESSION['DB'] == 'addedschools_lucknowips_testing') {
					$Conditions[] = 'afc.feeDate BETWEEN "2022-04-01" AND "2023-03-31"';
				} else if ($Filters['CollectionAcademicYearID'] == 1 && $_SESSION['DB'] == 'addedschools_lucknowips_testing-21-22') {
					$Conditions[] = 'afc.feeDate BETWEEN "2020-04-01" AND "2021-03-31"';
				} else if ($Filters['CollectionAcademicYearID'] == 2 && $_SESSION['DB'] == 'addedschools_lucknowips_testing-21-22') {
					$Conditions[] = 'afc.feeDate BETWEEN "2021-04-01" AND "2022-03-31"';
				} else if ($Filters['CollectionAcademicYearID'] == 3 && $_SESSION['DB'] == 'addedschools_lucknowips_testing-21-22') {
					$Conditions[] = 'afc.feeDate BETWEEN "2022-04-01" AND "2023-03-31"';
				}
			...
			if ($GetTotalsOnly) {
				$RSSearch = $DBConnObject->Prepare('SELECT afc.feeTransactionID, afc.feeCollectionID, afc.feeDate, 
													...
													afc.createDate, afpmd.paymentMode, CONCAT( YEAR(ay.startDate), \'-\', DATE_FORMAT(ay.endDate, \'%y\')) AS academicYearName, u.userName AS createUserName, 
													asd.firstName, asd.lastName, ac.className, asm.sectionName, aft.description, apd.fatherMobileNumber, apd.motherMobileNumber, asd.mobileNumber,
													afcoc.amount as late_fee, afcoc.feeType
			...
			$RSSearch = $DBConnObject->Prepare('SELECT asd.studentID, afc.feeTransactionID, afc.feeCollectionID, afc.feeDate, 
												...
												(SELECT SUM(amount) FROM afm_fee_collection_other_charges WHERE feeCollectionID = afc.feeCollectionID) AS previousYearAmountPaid, 
												afc.createDate, afpmd.paymentMode, CONCAT( YEAR(ay.startDate), \'-\', DATE_FORMAT(ay.endDate, \'%y\')) AS academicYearName, u.userName AS createUserName, 
    											asd.firstName, asd.lastName, ac.className, asm.sectionName, aft.description, apd.fatherMobileNumber, apd.motherMobileNumber, asd.mobileNumber, ass.enrollmentID,
    											afcoc.amount as late_fee, afcoc.feeType
    											FROM afm_fee_collection afc
												INNER JOIN afm_fee_transactions aft ON aft.feeTransactionID = afc.feeTransactionID
												INNER JOIN afm_fee_payment_mode_details afpmd ON afpmd.feeTransactionID = afc.feeTransactionID
			...
			while ($SearchRow = $RSSearch->FetchRow()) {
				...
				$FeeCollectionDetails['AcademicYearName'] = $SearchRow->academicYearName;
				$FeeCollectionDetails[$SearchRow->feeCollectionID]['EnrollmentID'] = $SearchRow->enrollmentID;
				$FeeCollectionDetails[$SearchRow->feeCollectionID]['FeeTransactionID'] = $SearchRow->feeTransactionID;
				$FeeCollectionDetails[$SearchRow->feeCollectionID]['StudentID'] = $SearchRow->studentID;
				$FeeCollectionDetails[$SearchRow->feeCollectionID]['StudentName'] = $SearchRow->firstName . ' ' . $SearchRow->lastName;
				...
				$FeeCollectionDetails[$SearchRow->feeCollectionID]['PreviousYearAmountPaid'] = 0;
				if ($SearchRow->feeType == 'LateFee') {
					$FeeCollectionDetails[$SearchRow->feeCollectionID]['AmountPaid'] = $SearchRow->amountPaid + $SearchRow->late_fee;
				} else {
					$FeeCollectionDetails[$SearchRow->feeCollectionID]['AmountPaid'] = $SearchRow->amountPaid;
				}
				$FeeCollectionDetails[$SearchRow->feeCollectionID]['FeeType'] = $SearchRow->feeType;
				$FeeCollectionDetails[$SearchRow->feeCollectionID]['LateFeeAmount'] = $SearchRow->late_fee;
		...
		try {
			...
			if (count($Filters) > 0) {
				...
				if ($_SESSION['DB'] == 'addedschools_lucknowips_testing-23-24') {
					$Conditions[] = 'ass.academicYearID != 3';
				}
			}
			...
			$RSSearch = $DBConnObject->Prepare('SELECT asd.studentID, asd.firstName, asd.lastName, ac.className, asm.sectionName, apd.fatherMobileNumber, apd.motherMobileNumber, ass.enrollmentID
			...
			while ($SearchRow = $RSSearch->FetchRow()) {
				...
				$SearchStudentFee = $DBConnObject->Prepare('SELECT
														...
														INNER JOIN asa_academic_year_months aaym ON afsd.academicYearMonthID = aaym.academicYearMonthID
														INNER JOIN afm_fee_heads afh ON afsd.feeHeadID = afh.feeHeadID
														WHERE ' . $query_condition . ' asfs.studentID = :|1 AND asfs.academicYearID = :|2 AND afh.isActive = 1;');
				...
				if ($PrevAcademicYearID > 0) {
					...
					// exit;
					$PreviousDueAmountses = 0;
					$PreviousDueAmountes = 0;
					$PreviousDueAmount = 0;
					$PreviousDueAmountes4 = 0;
					if (isset($_SESSION['DB']) && $_SESSION['DB'] == 'addedschools_lucknowips_testing-21-22') {
						$Year = 0;
						$PreviousDueAmount = FeeCollection::GetPreviousDefaulterAmount($SearchRow->studentID, $Year, $Filters['DueToDate']);
						$PreviousYearDueAmount += $PreviousDueAmount;
					} elseif (isset($_SESSION['DB']) && $_SESSION['DB'] == 'addedschools_lucknowips_testing-22-23') {
						$Year = 0;
						$PreviousDueAmountses += FeeCollection::GetPreviousDefaulterAmount($SearchRow->studentID, $Year, $Filters['DueToDate']);
						$PreviousDueAmountes += FeeCollection::GetPrevYear3DefaulterAmount($SearchRow->studentID, $Year, $Filters['DueToDate']);
						$PreviousYearDueAmount = $PreviousDueAmountes + $PreviousDueAmountses;
					} elseif (isset($_SESSION['DB']) && $_SESSION['DB'] == 'addedschools_lucknowips_testing-23-24') {
						$Year = 0;
						$PreviousDueAmountses += FeeCollection::GetPreviousDefaulterAmount($SearchRow->studentID, $Year, $Filters['DueToDate']);
						$PreviousDueAmountes += FeeCollection::GetPrevYear3DefaulterAmount($SearchRow->studentID, $Year, $Filters['DueToDate']);
						$PreviousDueAmountes4 += FeeCollection::GetPrevYear4DefaulterAmount($SearchRow->studentID, $Year, $Filters['DueToDate']);
						$PreviousYearDueAmount = $PreviousDueAmountes + $PreviousDueAmountses + $PreviousDueAmountes4;
					}
		...
		try {
			...
			while ($SearchRow = $RSFeeDefaulterDues->FetchRow()) {
				...
				if ($SecondDiscountAmount > 0) {
					...
					$FeeDefaulterDues[$SearchRow->monthName][$SearchRow->feeHeadID]['DiscountValue'] = $SearchRow->secondDiscountValue;
					if ($SearchRow->secondDiscountType == 'Percentage') {
						$FeeDefaulterDues[$SearchRow->monthName][$SearchRow->feeHeadID]['DiscountAmount'] = ($FeeAmount * $SearchRow->secondDiscountValue) / 100;
					} elseif ($SearchRow->secondDiscountType == 'Absolute') {
						$FeeDefaulterDues[$SearchRow->monthName][$SearchRow->feeHeadID]['DiscountAmount'] = $SecondDiscountAmount;
					}
				} else {
					$FeeHeadDiscountAmount = $FirstDiscountAmount;
					...
					$FeeDefaulterDues[$SearchRow->monthName][$SearchRow->feeHeadID]['DiscountValue'] = $SearchRow->firstDiscountValue;
					if ($SearchRow->secondDiscountType == 'Percentage') {
						$FeeDefaulterDues[$SearchRow->monthName][$SearchRow->feeHeadID]['DiscountAmount'] = ($FeeAmount * $SearchRow->firstDiscountValue) / 100;
					} elseif ($SearchRow->secondDiscountType == 'Absolute') {
						$FeeDefaulterDues[$SearchRow->monthName][$SearchRow->feeHeadID]['DiscountAmount'] = $FirstDiscountAmount;
					}
				}
				...
				if (($SearchRow->amountPayable - $FeeHeadDiscountAmount) === $SearchRow->amountPaid) {
					...
					$FeeHeadAmount += $FeeAmount - ($FeeDefaulterDues[$SearchRow->monthName][$SearchRow->feeHeadID]['DiscountAmount'] + $SearchRow->amountPaid);
	...
	static function SaveLateFee($StudentID, $MonthID, $LateFeeAmount)
	{
		try {
			$DBConnObject = new DBConnect();

			$RSSearch = $DBConnObject->Prepare('SELECT amount FROM student_late_fee WHERE studentID = :|1 AND monthID = :|2 AND status = "UnPaid";');
			$RSSearch->Execute($StudentID, $MonthID);

			if ($RSSearch->Result->num_rows <= 0) {
				$RSInsert = $DBConnObject->Prepare('INSERT INTO student_late_fee 
												(studentID, monthID, amount, payable_amount, createDateTime)
                                                VALUES (:|1, :|2, :|3, :|4, NOW());');
				$RSInsert->Execute($StudentID, $MonthID, $LateFeeAmount, $LateFeeAmount);
			} else {
				$RSInsert = $DBConnObject->Prepare('UPDATE student_late_fee 
														SET amount = :|1,
														payable_amount = :|2
														WHERE studentID = :|3 AND monthID =:|4 AND status = "UnPaid";');
				$RSInsert->Execute($LateFeeAmount, $LateFeeAmount, $StudentID, $MonthID);
			}

			return true;
		} catch (ApplicationDBException $e) {
			error_log('DEBUG: ApplicationDBException at FeeCollection::SaveLateFee(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at FeeCollection::SaveLateFee(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		}
	}

	static function SaveLateFeeColletionDetails($LateFeesDetails, $FeeCollectionID, $StudentID)
	{
		try {
			$DBConnObject = new DBConnect();

			$RSSearch = $DBConnObject->Prepare('SELECT feeCollectionOtherChargeID FROM `afm_fee_collection_other_charges` WHERE `feeCollectionID` = :|1 LIMIT 1;');
			$RSSearch->Execute($FeeCollectionID);

			$FeeCollectionOtherChargeID = $RSSearch->FetchRow()->feeCollectionOtherChargeID;

			$PaidAmount = 0;
			foreach ($LateFeesDetails as $LateFeeID => $Details) {
				$RSSearchDueAmount = $DBConnObject->Prepare('SELECT paid_amount 
																FROM student_late_fee
																WHERE LateFeeID = :|1 AND status = "Due";');
				$RSSearchDueAmount->Execute($LateFeeID);

				if ($RSSearchDueAmount->Result->num_rows > 0) {
					$PaidAmount = $RSSearchDueAmount->FetchRow()->paid_amount;

					$RSUpdate = $DBConnObject->Prepare('UPDATE student_late_fee  
															SET status = :|1,
															paid_amount = :|2
															WHERE lateFeeID = :|3;');
					$RSUpdate->Execute($Details['Status'], $Details['PaidAmount'] + $PaidAmount, $LateFeeID);
				} else {
					$RSUpdate = $DBConnObject->Prepare('UPDATE student_late_fee  
															SET status = :|1,
															paid_amount = :|2
															WHERE lateFeeID = :|3;');
					$RSUpdate->Execute($Details['Status'], $Details['PaidAmount'], $LateFeeID);
				}

				$RSInsert = $DBConnObject->Prepare('INSERT INTO afm_fee_collection_other_charges_details 
													(feeCollectionOtherChargeID, lateFeeID, createDate)
													VALUES (:|1, :|2, NOW());');
				$RSInsert->Execute($FeeCollectionOtherChargeID, $LateFeeID);
			}

			return true;
		} catch (ApplicationDBException $e) {
			error_log('DEBUG: ApplicationDBException at FeeCollection::SaveLateFeeColletionDetails(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at FeeCollection::SaveLateFeeColletionDetails(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		}
	}

	static function SearchLateFee($StudentID)
	{
		$LateFeeDetails = array();
		try {
			$DBConnObject = new DBConnect();

			$RSSearch = $DBConnObject->Prepare('SELECT slf.*, aaym.monthName
												FROM student_late_fee  slf
												INNER JOIN asa_academic_year_months aaym ON slf.monthID = aaym.academicYearMonthID
												WHERE slf.studentID = :|1 AND (slf.status = "UnPaid" OR slf.status = "Due")
			...
			while ($SearchRow = $RSSearch->FetchRow()) {
				$LateFeeDetails[$SearchRow->lateFeeID]['Status'] = $SearchRow->status;
				if ($SearchRow->status == 'Due') {
					$LateFeeDetails[$SearchRow->lateFeeID]['Amount'] = $SearchRow->amount - $SearchRow->paid_amount;
				} else {
					$LateFeeDetails[$SearchRow->lateFeeID]['Amount'] = $SearchRow->amount;
				}
				$LateFeeDetails[$SearchRow->lateFeeID]['MonthID'] = $SearchRow->monthID;
				$LateFeeDetails[$SearchRow->lateFeeID]['MonthName'] = $SearchRow->monthName;
				$LateFeeDetails[$SearchRow->lateFeeID]['PaidAmount'] = 0;
			}

			return $LateFeeDetails;
		} catch (ApplicationDBException $e) {
			error_log('DEBUG: ApplicationDBException at FeeCollection::SearchLateFee(). Stack Trace: ' . $e->getTraceAsString());
			return $LateFeeDetails;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at FeeCollection::SearchLateFee(). Stack Trace: ' . $e->getTraceAsString());
			return $LateFeeDetails;
		}
	}
		...
		if ($RSSelectOtherFeeType->Result->num_rows > 0) {
			while ($SearchRow = $RSSelectOtherFeeType->FetchRow()) {
				if ($SearchRow->feeType == 'PreviousYearDue') {
					...
					$RSSearchPreviousYearFeeDetailID->Execute($this->StudentID);
