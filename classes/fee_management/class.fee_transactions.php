...
class FeeTransaction
{
	...
	private function RemoveFeeTransaction()
    {
        ...
        $RSSearchFeeCollection = $this->DBObject->Prepare('SELECT * FROM afm_fee_collection WHERE feeTransactionID = :|1;');
        ...
        if ($RSSearchFeeCollection->Result->num_rows > 0)
        {
            ...
            {	
				...
				$RSSearch = $this->DBObject->Prepare('SELECT * FROM afm_fee_collection_details WHERE feeCollectionID = :|1;');
				$RSSearch->Execute($SearchFeeCollectionRow->feeCollectionID);

				if ($RSSearch->Result->num_rows > 0)
				{
					$json[$jsonRecordCounter]['table'] = 'afm_fee_collection_details';
		        ...
		        if ($RSSelectOtherFeeType->Result->num_rows > 0)
		        {
		            ...
		            {	
		                if ($SearchRow->feeType == 'PreviousYearDue')
		                {
							...
							$json[$jsonRecordCounter]['table'] = 'afm_previous_year_fee_details';
							$json[$jsonRecordCounter]['records'] = 'subtract paidAmount: '.$SearchRow->amount;
							...
							$RSSearch = $this->DBObject->Prepare('SELECT * FROM afm_previous_year_fee_details_transactions WHERE previousYearFeeDetailID = :|1 AND paidAmount = :|2;');
							$RSSearch->Execute($PreviousYearFeeDetailID, $SearchRow->amount);

							if ($RSSearch->Result->num_rows > 0)
							{
								$json[$jsonRecordCounter]['table'] = 'afm_previous_year_fee_details_transactions';
								$json[$jsonRecordCounter]['records'] = json_encode($RSSearch->FetchRow());
								$json[$jsonRecordCounter]['action'] = 'DELETE';
								$jsonRecordCounter++;
							}
				...
				$RSSearch = $this->DBObject->Prepare('SELECT * FROM afm_fee_collection_other_charges WHERE feeCollectionID = :|1;');
				$RSSearch->Execute($SearchFeeCollectionRow->feeCollectionID);

				if ($RSSearch->Result->num_rows > 0)
				{
					$json[$jsonRecordCounter]['table'] = 'afm_fee_collection_other_charges';
					$json[$jsonRecordCounter]['records'] = json_encode($RSSearch->FetchRow());
					$json[$jsonRecordCounter]['action'] = 'DELETE';
					$jsonRecordCounter++;
				}
		...
		$RSSearch = $this->DBObject->Prepare('SELECT * FROM afm_fee_collection WHERE feeTransactionID = :|1;');
		$RSSearch->Execute($this->FeeTransactionID);

		if ($RSSearch->Result->num_rows > 0)
		{
			$json[$jsonRecordCounter]['table'] = 'afm_fee_collection';
			$json[$jsonRecordCounter]['records'] = json_encode($RSSearch->FetchRow());
			$json[$jsonRecordCounter]['action'] = 'DELETE';
			$jsonRecordCounter++;
		}
		...
		$RSSearch = $this->DBObject->Prepare('SELECT * FROM afm_fee_transactions WHERE feeTransactionID = :|1;');
		$RSSearch->Execute($this->FeeTransactionID);

		if ($RSSearch->Result->num_rows > 0)
		{
			$json[$jsonRecordCounter]['table'] = 'afm_fee_transactions';
			$json[$jsonRecordCounter]['records'] = json_encode($RSSearch->FetchRow());
			$json[$jsonRecordCounter]['action'] = 'DELETE';
			$jsonRecordCounter++;
		}
        ...
        {
			$AdvanceAmount = $RSSearchAdvanceAmount->FetchRow()->advanceAmount;

			$json[$jsonRecordCounter]['table'] = 'asa_parent_details';
			$json[$jsonRecordCounter]['records'] = 'subtract walletAmount using parent id: '.$AdvanceAmount;
			$json[$jsonRecordCounter]['action'] = 'UPDATE';
			$jsonRecordCounter++;
    		...
    		$RSUpdate->Execute($AdvanceAmount, $StudentID);
			
			$RSSearch = $this->DBObject->Prepare('SELECT * FROM afm_advance_fee WHERE feeTransactionID = :|1;');
			$RSSearch->Execute($this->FeeTransactionID);

			if ($RSSearch->Result->num_rows > 0)
			{
				$json[$jsonRecordCounter]['table'] = 'afm_advance_fee';
				$json[$jsonRecordCounter]['records'] = json_encode($RSSearch->FetchRow());
				$json[$jsonRecordCounter]['action'] = 'DELETE';
				$jsonRecordCounter++;
			}
		...
		foreach ($json as $data)
		{
			...
			$RSInsertLog->Execute($_SESSION['ValidUser'], 'FeeTransactionID: '.$this->FeeTransactionID, $data['table'], $data['records'], $data['action']);
		}

        return true;
    }
    
    private function RemoveFeeTransactionOld()
    {
        if(!isset($this->FeeTransactionID)) 
        {
            throw new ApplicationARException('', APP_AR_ERROR_DELETE_WITHOUT_ID);
