...
class FeeHead
{
	...
	static function GetActiveFeeHeads($sortByPriority = 'priority ASC')
	{
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT * FROM afm_fee_heads WHERE isActive = 1 ORDER BY '.$sortByPriority.';');
			$RSSearch->Execute();
