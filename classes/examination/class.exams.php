...
class Exam
{
	...
	static function GetAllExamsForReportCard($ClassSectionID, $GetNameOnly = true, $SelectedExams = '', $AcademicYearID = 2)
	{
		$AllExams = array();
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT DISTINCT(ae.examTypeID), ae.examName, ae.examID, aet.examType
												...
												WHERE ae.classSectionID = :|1 ' . $QueryString . ' AND ae.academicYearID = :|2 GROUP BY (ae.examTypeID);');
	...
	static function GetAllExamSubjectGrandTotals($StudentID, $AcademicYearID = 2)
	{
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT astem.studentExamMarkID, SUM(astem.marks) AS grandTotalOfMarks, SUM(ae.maximumMarks)  AS grandTotalOfMaximumMarks
												...
												LEFT JOIN aem_exams ae ON astem.examID = ae.examID
												LEFT JOIN aem_exam_types aet ON ae.examTypeID = aet.examTypeID
												WHERE ast.studentID = :|1 AND academicYearID = :|2 GROUP BY ae.examTypeID ORDER BY ae.examTypeID;');
	...
	static function GetTotalMarksOfExam($ExamTypeID, $ClassSectionID, $AcademicYearID = 2)
	{
		$ExamTotal = 0;
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT SUM(maximumMarks)  AS examTotal
												FROM aem_exams
												WHERE examTypeID = :|1 AND classSectionID = :|2 AND academicYearID = :|3;');
			$RSSearch->Execute($ExamTypeID, $ClassSectionID, $AcademicYearID);
	...
	static function SearchExams(&$TotalRecords = 0, $GetTotalsOnly = false, $Filters = array(), $Start = 0, $Limit = 100)
	{
		...
		try
		{
			...
			if (count($Filters) > 0)
			{
				...
				{
					$Conditions[] = 'ae.academicYearID = ' . $DBConnObject->RealEscapeVariable($Filters['AcademicYearID']);
				}
			}
	...
	static function GetExamApplicableClasses($ExamTypeID, $AcademicYearID = 2)
	{
		$AllClasses = array();
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT DISTINCT(ae.classSectionID), acs.classID, ac.className 
												...
												WHERE ae.examTypeID = :|1 AND ae.academicYearID = :|2 ORDER BY ac.classID;');
			$RSSearch->Execute($ExamTypeID, $AcademicYearID);
	...
	static function GetExamApplicableClassSections($ExamTypeID, $ClassID, $AcademicYearID = 2)
	{
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT acs.classSectionID, asm.sectionName
												...
												WHERE ae.examTypeID = :|1 AND acs.classID = :|2 AND ae.academicYearID = :|3 GROUP BY asm.sectionMasterID, ae.examTypeID;');
			$RSSearch->Execute($ExamTypeID, $ClassID, $AcademicYearID);
	...
	static function GetExamSubjects($ExamTypeID, $ClassSectionID, $StudentID = 0, $GetNameOnly = false, $AcademicYearID = 2)
	{
		$AllSubjects = array();
		...
		try
		{
			...
			if ($GetNameOnly)
			{
				$RSSearch = $DBConnObject->Prepare('SELECT DISTINCT(ae.classSubjectID), asm.subject, ae.maximumMarks, 
													...
													WHERE ae.examTypeID = :|1 AND ae.classSectionID = :|2 AND ae.academicYearID = :|3 ORDER BY asm.subject;');
				$RSSearch->Execute($ExamTypeID, $ClassSectionID, $AcademicYearID);
			...
			$RSSearch = $DBConnObject->Prepare('SELECT DISTINCT(ae.classSectionID), ae.examID, ae.maximumMarks, ae.classSubjectID, 
												...
												INNER JOIN asa_subject_master asm ON acs.subjectID = asm.subjectID
												LEFT JOIN aem_student_exam_marks asem ON ae.examID = asem.examID
												WHERE ae.examTypeID = :|1 AND ae.classSectionID = :|2 AND asem.studentID = :|3 AND ae.academicYearID = :|4;');
			$RSSearch->Execute($ExamTypeID, $ClassSectionID, $StudentID, $AcademicYearID);
	...
	static function GetReportCardSubject($ClassSectionID, $AcademicYearID = 2)
	{
		$AllSubjects = array();
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT DISTINCT(ae.classSubjectID)
													FROM aem_exams ae
													WHERE ae.classSectionID = :|1 AND ae.academicYearID = :|2;');
			$RSSearch->Execute($ClassSectionID, $AcademicYearID);
	...
	static function GetOfflineExams($AcademicYearID = 2)
	{
		$OfflineExams = array();
		...
		try
		{
			...
			$RSSearch = $DBConnObject->Prepare('SELECT examID, examName FROM aem_exams WHERE isOffline = 1 AND academicYearID = :|2;');
	...
	private function SaveDetails()
	{
		if ($this->ExamID == 0)
		{
			foreach ($this->ClassSectionList as $ClassSectionID => $ClassSectionID)
			{
				...
				{
					$RSSave = $this->DBObject->Prepare('INSERT INTO aem_exams (academicYearID, examTypeID, classSectionID, classSubjectID, examName, maximumMarks, isOffline, createUserID, createDate)
																VALUES (:|1, :|2, :|3, :|4, :|5, :|6, :|7, :|8, NOW());');

					$RSSave->Execute($this->AcademicYearID, $this->ExamTypeID, $ClassSectionID, $ClassSubjectID, $this->ExamName, $MaximumMarks, $this->IsOffline, $this->CreateUserID);
				}
			}
		}
		else
		{
			$RSUpdate = $this->DBObject->Prepare('UPDATE aem_exams
													SET academicYearID = :|1,
														...
														maximumMarks = :|6,
														isOffline = :|7
													WHERE examID = :|8 LIMIT 1;');

			$RSUpdate->Execute($this->AcademicYearID, $this->ExamTypeID, $this->ClassSectionID, $this->ClassSubjectID, $this->ExamName, $this->MaximumMarks, $this->IsOffline, $this->ExamID);
		}
	...
	private function RemoveExam()
	{
		...
		$RSSearch = $this->DBObject->Prepare('SELECT * FROM aem_exams WHERE examID = :|1;');
		$RSSearch->Execute($this->ExamID);

		if ($RSSearch->Result->num_rows > 0)
		{
			$json[$jsonRecordCounter]['table'] = 'aem_exams';
		...
		foreach ($json as $data)
		{
			...
			$RSInsertLog->Execute($_SESSION['ValidUser'], 'ExamID: ' . $this->ExamID, $data['table'], $data['records'], $data['action']);
		}
	...
	private function SetAttributesFromDB($ExamRow)
	{
		...
		$this->AcademicYearID = $ExamRow->academicYearID;
