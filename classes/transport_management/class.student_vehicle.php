...
class StudentVehicle
{
	...
	static function SearchStudentVehicles(&$TotalRecords = 0, $GetTotalsOnly = false, $Filters = array(), $Start = 0, $Limit = 100)
	{
		...
		try {
			...
			$RSSearch = $DBConnObject->Prepare('SELECT asv.*, ar.routeNumber, ar.routeName, aam.areaName, asd.firstName, asd.lastName, asd.dob, asd.address1, asd.address2, acl.className, asm.sectionName, apd.fatherFirstName, apd.fatherLastName, apd.fatherMobileNumber, 
			                                    apd.motherMobileNumber, aawf.amount, u.userName AS createUserName, ass.studentID, ass.enrollmentID
												FROM atm_student_vehicle asv
												INNER JOIN atm_area_wise_fee aawf ON aawf.areaWiseFeeID = asv.areaWiseFeeID
												INNER JOIN atm_area_master aam ON aawf.areaID = aam.areaID
												INNER JOIN atm_routes ar ON ar.routeID = aawf.routeID
			...
			while ($SearchRow = $RSSearch->FetchRow()) {
				$AllStudentVehicles[$SearchRow->studentVehicleID]['StudentID'] = $SearchRow->studentID;
				$AllStudentVehicles[$SearchRow->studentVehicleID]['EnrollmentID'] = $SearchRow->enrollmentID;
	...
	static function GetStudentTransportChangeLogByID($StudentID = 0)
	{
		...
		try {
			...
			$RSSearch = $DBConnObject->Prepare('SELECT stcl.*, asd.firstName, asd.lastName
												FROM asa_student_transport_change_log stcl
												INNER JOIN asa_students s ON s.studentID = stcl.studentID
												INNER JOIN asa_student_details asd ON asd.studentID = stcl.studentID
												WHERE stcl.studentID = :|1;');
			$RSSearch->Execute($StudentID);

			if ($RSSearch->Result->num_rows <= 0) {
				return $StudentTransportChangeLog;
			}

			while ($SearchRow = $RSSearch->FetchRow()) {
				$StudentTransportChangeLog[$SearchRow->studentTransportChangeLogID]['FirstName'] = $SearchRow->firstName;
				$StudentTransportChangeLog[$SearchRow->studentTransportChangeLogID]['LastName'] = $SearchRow->lastName;
				$StudentTransportChangeLog[$SearchRow->studentTransportChangeLogID]['NewAmount'] = $SearchRow->newAmount;
				$StudentTransportChangeLog[$SearchRow->studentTransportChangeLogID]['OldAmount'] = $SearchRow->oldAmount;
				$StudentTransportChangeLog[$SearchRow->studentTransportChangeLogID]['UserName'] = $SearchRow->userName;
				$StudentTransportChangeLog[$SearchRow->studentTransportChangeLogID]['CurrentDate'] = $SearchRow->currentDate;
				$StudentTransportChangeLog[$SearchRow->studentTransportChangeLogID]['CreateDateTime'] = $SearchRow->createDateTime;
			}

			return $StudentTransportChangeLog;
		} catch (ApplicationDBException $e) {
			error_log('DEBUG: ApplicationDBException at StudentVehicle::GetStudentTransportChangeLogByID(). Stack Trace: ' . $e->getTraceAsString());
			return $StudentTransportChangeLog;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at StudentVehicle::GetStudentTransportChangeLogByID(). Stack Trace: ' . $e->getTraceAsString());
			return $StudentTransportChangeLog;
		}
	}
	...
	private function SaveDetails()
	{
		if ($this->StudentVehicleID == 0) {
			...
			if ($RSSearchFeeHeadID->Result->num_rows > 0) {
				...
				$RSSearchFeeStructureDetails = $this->DBObject->Prepare('SELECT feeStructureDetailID, academicYearID FROM afm_fee_structure_details WHERE feeStructureID = :|1 AND feeHeadID = :|2;');
				...
				if ($RSSearchFeeStructureDetails->Result->num_rows > 0) {
					while ($SearchFeeStructureDetailsRow = $RSSearchFeeStructureDetails->FetchRow()) {
						$RSSaveTransportFee = $this->DBObject->Prepare('INSERT INTO afm_student_fee_structure (academicYearID, feeStructureDetailID, StudentID, amountPayable)
																		VALUES (:|1, :|2, :|3, :|4);');

						$RSSaveTransportFee->Execute($SearchFeeStructureDetailsRow->academicYearID, $SearchFeeStructureDetailsRow->feeStructureDetailID, $this->StudentID, $AreaAmount);
					}
				}
			}
		} else {
			$OldAmount = 0;
			$RSPrevTransportAmount = $this->DBObject->Prepare('SELECT DISTINCT(asfs.amountPayable)
															...
															WHERE asfs.studentID = :|1 AND afsd.feeHeadID = 6');
			$RSPrevTransportAmount->Execute($this->StudentID);

			$OldAmount = $RSPrevTransportAmount->FetchRow()->amountPayable;
			...
			if ($RSSearchFeeHeadID->Result->num_rows > 0) {
				...
				if ($_SESSION['DB'] == 'addedschools_lucknowips_testing-23-24') {
					$AcademicYearID = 4;
				} else if ($_SESSION['DB'] == 'addedschools_lucknowips_testing-22-23') {
					$AcademicYearID = 3;
				} else {
					$AcademicYearID = 2;
				}

				$RSStudentTransportChangeLog = $this->DBObject->Prepare('INSERT INTO asa_student_transport_change_log
																	(studentID, academicYearID, currentDate, userName, newAmount, oldAmount, createUserID, createDateTime)
																	VALUES(:|1, :|2, :|3, :|4, :|5, :|6, :|7, NOW());');
				$RSStudentTransportChangeLog->Execute($this->StudentID, $AcademicYearID, $CurrentDate, $_SESSION['ValidUser'], $AreaAmount, $OldAmount, $this->CreateUserID);
			}
		}
	...
	private function RemoveStudentVehicle()
	{
		...
		$RSINSERTDELETELOG = $this->DBObject->Prepare('INSERT INTO asa_student_transport_change_log
													(studentID, academicYearID, currentDate, userName, newAmount, oldAmount, actionStatus, createUserID, createDateTime)
													VALUES((SELECT studentID FROM atm_student_vehicle WHERE studentVehicleID = :|1), :|2, :|3, :|4, 0, 0, :|5, :|6, NOW());');
		$RSINSERTDELETELOG->Execute($this->StudentVehicleID, $AcademicYearID, $CurrentDate, $_SESSION['ValidUser'], $ActionStatus, $this->CreateUserID);
