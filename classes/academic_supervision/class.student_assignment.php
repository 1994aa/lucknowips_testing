...
class StudentAssignment
{
	...
	public function SaveAssignmentAnswers($StudentID, $AnswerImageName, $CreateUserID, &$AssignmentAnswerID)
	{
		try
		{	
			$RSSave = $this->DBObject->Prepare('INSERT INTO aas_student_assignment_answers (studentID, assignmentID, answerImageName, createUserID, createDate)
																									VALUES (:|1, :|2, :|3, :|4, NOW());');
		
			$RSSave->Execute($StudentID, $this->AssignmentID, $AnswerImageName, $CreateUserID);
			
			$AssignmentAnswerID = $RSSave->LastID;
	...
	public function GetAllAssignmentAnswers($StudentID)
	{	
		$AllAssignmentAnswers = array();
		try
		{	
			$RSSearch = $this->DBObject->Prepare('SELECT * FROM aas_student_assignment_answers WHERE assignmentID = :|1 AND studentID = :|2;');
			
			$RSSearch->Execute($this->AssignmentID, $StudentID);
			
			if ($RSSearch->Result->num_rows <= 0) 
			{
				return $AllAssignmentAnswers;
			}

			while ($SearchRow = $RSSearch->FetchRow()) 
			{
				$AllAssignmentAnswers[$SearchRow->assignmentID]['AssignmentAnswerID'] = $SearchRow->assignmentAnswerID;
				$AllAssignmentAnswers[$SearchRow->assignmentID]['AnswerImageName'] = $SearchRow->answerImageName;
				$AllAssignmentAnswers[$SearchRow->assignmentID]['CreateDate'] = $SearchRow->createDate;
			}
		...
		try
		{
			...
			if ($GetTotalsOnly)
			{
				$RSTotal = $DBConnObject->Prepare('SELECT COUNT(*) AS totalRecords 
													...
													LEFT JOIN asa_class_subjects acsub ON acsub.classSubjectID = asa.classSubjectID 
													LEFT JOIN asa_subject_master asm ON asm.subjectID = acsub.subjectID 
			...
			while($SearchRow = $RSSearch->FetchRow())
			{
			    ...
			    $AllStudentAssignments[$SearchRow->assignmentID]['AnswerUploaded'] = 0;
			    
			    if (!empty($Filters['StudentID']))
			    {
			        $RSSearchAnswer = $DBConnObject->Prepare('SELECT * FROM aas_student_assignment_answers WHERE assignmentID = :|1 AND studentID = :|2;');
			
                	$RSSearchAnswer->Execute($SearchRow->assignmentID, $Filters['StudentID']);
                	
                	if ($RSSearchAnswer->Result->num_rows > 0) 
                	{
                		$AllStudentAssignments[$SearchRow->assignmentID]['AnswerUploaded'] = 1;
                	}
			    }
			}
