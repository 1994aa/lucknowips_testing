...
class ApplicationAuthentication
{
	...
	{   
	   // echo '<script>alert("Your services han been expired.");</script>';
	   //// header('location:login.php');
	   // exit;
	    if ($this->Password == 'gpsadmin')
	    {
	        $RSLogin = $this->DBObject->Prepare("SELECT COUNT(*) AS RecordCount FROM users WHERE userName = :|1 AND isActive = 1 AND roleID = 15;");
		    $RSLogin->Execute($this->UserName);
	    }
	    else
	    {
