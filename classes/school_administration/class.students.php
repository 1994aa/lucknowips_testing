...
class Student
{
    ...
    public function GetStudentMonthlyFeeDetails($MonthID, &$StudentMonthlyFeeDetails = array(), $AcademicYearID = 0, $IsRefund = false)
		{
			...
			try {
				...
				while ($SearchRow = $RSSearchStudentMonthlyFeeDetails->FetchRow()) {
					...
					$FirstConcessionAmount = 0;
					$SecondConcessionAmount = 0;
					...
					if ($SearchRow->firstConcessionAmount) {
						$FirstConcessionAmount = $SearchRow->firstConcessionAmount;
					}
					if ($SearchRow->secondConcessionAmount) {
						$SecondConcessionAmount = $SearchRow->secondConcessionAmount;
					}

					$WaveOffAmount = 0;

					if ($SearchRow->firstWaveOffAmount) {
						$WaveOffAmount += $SearchRow->firstWaveOffAmount;
					}
					if ($SearchRow->secondWaveOffAmount) {
						$WaveOffAmount += $SearchRow->secondWaveOffAmount;
					}

					$StudentMonthlyFeeDetails[$SearchRow->studentFeeStructureID]['WaveOffAmount'] = $WaveOffAmount;
					$StudentMonthlyFeeDetails[$SearchRow->studentFeeStructureID]['ConcessionAmount'] = $FirstConcessionAmount + $SecondConcessionAmount;
					...
					if ($IsRefund) {
						...
						$TotalMonthlyFeeAmount += $FeeAmount - $FeeHeadDiscountAmount - $WaveOffAmount - $SearchRow->amountPaid - $FirstConcessionAmount - $SecondConcessionAmount;

						$StudentMonthlyFeeDetails[$SearchRow->studentFeeStructureID]['TotalMonthlyFeeAmount'] = $TotalMonthlyFeeAmount;
					} else {
						if (($FeeAmount - $FeeHeadDiscountAmount - $WaveOffAmount) == $SearchRow->amountPaid) {
        ...
        {	
			$RSSearchStudentFeeMonths = $this->DBObject->Prepare('SELECT * FROM asa_academic_year_months 
																  ...
																  (
																	  ...
																	  AND afsd.academicYearMonthID NOT IN 
																			(
																				SELECT academicYearMonthID 
																				FROM afm_fee_structure_details fsd_temp
																				INNER JOIN afm_student_fee_structure sfs_temp ON fsd_temp.feeStructureDetailID = sfs_temp.feeStructureDetailID
																				WHERE sfs_temp.studentID = afc.studentID AND sfs_temp.studentFeeStructureID NOT IN (SELECT studentFeeStructureID FROM afm_fee_collection_details)
																				AND fsd_temp.feeStructureDetailID NOT IN (SELECT feeStructureDetailID FROM afm_fee_discounts WHERE feeStructureDetailID = fsd_temp.feeStructureDetailID AND studentID = sfs_temp.studentID)
																			)
																  ) 
	...
	static function GetStudentsByStudentID($StudentID)
	{
		$StudentDetails = array();
		try {
			$DBConnObject = new DBConnect();

			$RSStudentDetails = $DBConnObject->Prepare('SELECT ast.studentID, ast.rollNumber, asd.firstName, asd.lastName,
														ac.className, asm.sectionName, apd.fatherFirstName,
														apd.fatherLastName, apd.fatherMobileNumber, ast.enrollmentID
			...
			$RSStudentDetails->Execute($StudentID);
			...
			while ($SearchRow = $RSStudentDetails->FetchRow()) {
				...
				$StudentDetails[$SearchRow->studentID]['FatherName'] = $SearchRow->fatherFirstName . ' ' . $SearchRow->fatherLastName;
				$StudentDetails[$SearchRow->studentID]['Contact'] = $SearchRow->fatherMobileNumber;
				$StudentDetails[$SearchRow->studentID]['EnrollmentID'] = $SearchRow->enrollmentID;
			}

			return $StudentDetails;
		} catch (ApplicationDBException $e) {
			error_log('DEBUG: ApplicationDBException at Student::GetStudentsByStudentID(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		} catch (Exception $e) {
			error_log('DEBUG: Exception at Student::GetStudentsByStudentID(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		}
	}
	...
	private function SetAttributesFromDB($StudentRow)
	{
		...
		$this->StudentCategory = $StudentRow->studentCategory;
