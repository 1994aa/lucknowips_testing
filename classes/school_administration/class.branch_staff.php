...
class BranchStaff
{
    ...
    static function SearchBranchStaff(&$TotalRecords = 0, $GetTotalsOnly = false, $Filters = array(), $Start = 0, $Limit = 100)
    {
    	...
    	try
		{
			...
			while($RSAllBranchStaffRow = $RSAllBranchStaff->FetchRow())
			{
				...
				$AllBranchStaff[$RSAllBranchStaffRow->branchStaffID]['Address1'] = $RSAllBranchStaffRow->address1;
	...
	private function RemoveBranchStaff()
	{
        ...
        $RSSearch = $this->DBObject->Prepare('SELECT * FROM asa_branch_staff WHERE branchStaffID = :|1;');
		$RSSearch->Execute($this->BranchStaffID);

		if ($RSSearch->Result->num_rows > 0)
		{
			$json[$jsonRecordCounter]['table'] = 'asa_branch_staff';
        ...
        foreach ($json as $data)
		{
			...
			$RSInsertLog->Execute($_SESSION['ValidUser'], 'BranchStaffID: '.$this->BranchStaffID, $data['table'], $data['records'], $data['action']);
		}
	}
