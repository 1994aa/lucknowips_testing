...
class ClassClassteacher
{
    ...
    { 
        ...
        {	
        	...
        	$RSSearch = $DBConnObject->Prepare('SELECT acc.*, ac.className, asm.sectionName, abs.firstName, abs.lastName, abs.dob, abs.userName, u.userName AS createUserName 
			...
			while($SearchRow = $RSSearch->FetchRow())
			{
				...
				$AllClassClassteachers[$SearchRow->classClassTeacherID]['UserName'] = $SearchRow->userName;
				
				$DOB = date('Y', strtotime($SearchRow->dob));
	
	            $Password = strtoupper(substr($SearchRow->firstName . $SearchRow->lastName, 0, 4)) . $DOB;
				$AllClassClassteachers[$SearchRow->classClassTeacherID]['Password'] = $Password;
