...
class ParentDetail
{
	...
	public function __construct($ParentID = 0, $UserName = '')
	{
		...
		else if ($UserName != '')
		{
			...
			$this->GetParentDetailByUserName();
		}
		else
		{
	...
	private function GetParentDetailByUserName()
	{
		$RSParentDetail = $this->DBObject->Prepare('SELECT * FROM asa_parent_details WHERE userName = :|1 LIMIT 1;');
		$RSParentDetail->Execute($this->UserName);
