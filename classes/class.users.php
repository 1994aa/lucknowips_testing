...
class User
{
	...
	public function ChangePasswordByAdmin($NewPassword, $UserName)
	{
		try
		{
			$RSUpdate = $this->DBObject->Prepare('UPDATE users SET password = SHA1(:|1) WHERE userName = :|2 LIMIT 1;');
			$RSUpdate->Execute($NewPassword, $UserName);
			
			return true;
		}
		catch (ApplicationDBException $e)
		{
			error_log('DEBUG: ApplicationDBException at User::ChangePasswordByAdmin(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		}
		catch (Exception $e)
		{
			error_log('DEBUG: Exception at User::ChangePasswordByAdmin(). Stack Trace: ' . $e->getTraceAsString());
			return false;
		}
	}
